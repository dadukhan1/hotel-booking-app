...
const AllRooms = () => {
  ...
  const handleFilterChange = (checked, value, type) => {
    setSelectedFilters((prevFilters) => {
      const udpatedFilters = { ...prevFilters };
      if (checked) {
        udpatedFilters[type].push(value);
      } else {
        udpatedFilters[type] = udpatedFilters[type].filter(item => item !== value);
      }
  ...
  // function to srot room based on the selected sort option
  const sortRooms = (a, b) => {
  ...
  // filter and sort rooms based ont he selected filters and sort option
  const filteredRooms = useMemo(() => {
    return rooms.filter(room => matchesRoomType(room) && mathcesPriceRange(room) && filterDestination(room)).sort(sortRooms);
      ...
      <div>
        <div>
          <Title
            title={"Hotel Rooms"}
            subTitle={
              "Take advantage of our limited-time offers and speical packages to enhance your stay and create unforgettable memories."
            }
            align={"left"}
          />
